<div class="row">
    <div class="col-lg-12">
        <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
            ...
            <a href="javascript:void(0)" class="btn btn-primary add-category"><i class="las la-plus mr-3"></i>Add
                Category</a>
        </div>
    </div>
    <div class="col-md-12">
        <div class="table-responsive rounded col-md-12 pl-0">
            <table class="data-table table mb-0 tbl-server-info" id="category-table">
                ...
                <form class="form-add-entry">
                    ...
                    <div class="modal-body">
                        <div class="modal-form">
                            ...
                            <label for="input_category">Category Name</label>
                            <input type="text" id="input_category" name="cat_name" class="form-control form-input" required >
                            </div>
                        </div>
                    </div>
        ...
        const brand = {
            ...
            event: function(){
                ...
                $('.add-category').click(function(){
                    utils.modal.open(__self.modal.add);
                })
                    ...
                    form_input.each(function(i){
                        ...
                        if(i == (count - 1)){
                            if(Object.keys(input).length){
                                utils.request.set({data: input, url: "/api/category/add"}).send(function(res){
                                    if(res.status){
                                        utils.notify.setTitle("Success").setMessage("Successfully added new category").setType("success").load();
                                        $(__self.form.add).trigger('reset');
                                        utils.modal.close(__self.modal.add);
                                        __self.loadTable();
                                    }else{
                                        utils.notify.setTitle("Error").setMessage(res.message).setType("success").load();
                                    }
                                })
