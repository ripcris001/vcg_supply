const utils = {
    ...
    request: {
        ...
        get: function(callback){
            const __self = this;
            callback = callback ? callback : function(){}
            jQuery.get(this.url).done(function(res){
                __self.reset();
                if(res){
                    res = JSON.parse(res);
                }
        ...
        post: function(callback){
            const __self =  this;
            callback = callback ? callback : function(){}
            jQuery.post(this.url, this.input).done(function(res){
                __self.reset();
                if(res){
                    res = JSON.parse(res);
                }
        ...
        put: function(callback){
            const __self =  this;
            callback = callback ? callback : function(){}
            jQuery.get(this.url, this.input).done(function(res){
                __self.reset();
                if(res){
                    res = JSON.parse(res);
                }
    ...
    notify: {
        ...
        load: function() {
            const local = this;
            jQuery.notify({ title: local.title, message: local.message }, local.config);
        }
    },
    removeError: function(){
        jQuery('body').on('input', '.form-error', function(){
    ...
    requreHandler: function(){
        jQuery('body').find('.form-input.form-required').each(function(){
            const local = jQuery(this);
